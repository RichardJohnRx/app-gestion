    ...
    if (this.numbers[n - 1].correct) {
      ...
      Swal.fire({
        ...
        .then(() => {
          ...
          this.hint =
            correctNumber < n
              ? 'El número debe ser menor que ' + n
              : 'El número debe ser mayor que ' + n;
        ...
        .then(() => {
          let availableNumbers = this.numbers.filter(
            (number: any) => number.disabled == false
          );
    ...
    if (this.puntuacion > 0) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this._gServ
            .setScore({ name: result.value, score: this.puntuacion })
            .then(() => {
              Swal.fire(
                'Guardado!',
